...
class MLA(nn.Module):
  ...
  def forward(self, X1, X2):
    alpha = torch.reshape(
      torch.nn.functional.softmax(torch.flatten(torch.nn.functional.tanh(self.conv2(X2)), start_dim=1),dim=1),
      (X2.shape[0], 1, X2.shape[-2], X2.shape[-1]))
    d_X1  =torch.nn.functional.interpolate(self.conv1(X1), (X2.shape[-2], X2.shape[-1]),
